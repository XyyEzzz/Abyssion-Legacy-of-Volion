...
export default function DialogueModal() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      // OS auto-repeat must not walk the pages: one advance per press.
      if (e.repeat) return;
      if (e.key === 'Escape') {
        closeDialogue();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center pb-8 sm:pb-12 px-4 pointer-events-auto bg-black backdrop-blur-[2px]">
      <div
      ...
      >
          ...
          <div className="flex gap-2 pt-2">
            {choices.map((choice, idx) => (
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  // Replace pages with choice result pages
                  useGameStore.setState((state) => ({
                    activeDialogue: state.activeDialogue
                      ? {
                          ...state.activeDialogue,
                          pages: choice.resultPages,
                          currentPage: 0,
                          questToOffer: undefined,
                          questToTurnIn: undefined,
                          choices: undefined,
                        }
                      : null,
                  }));
                  if (choice.openShop && activeDialogue) {
                    openShop(activeDialogue.npcId);
                  }
                  if (choice.chooseArchetype) {
                    useGameStore.getState().chooseArchetype(choice.chooseArchetype);
                  }
                }}
                className="px-4 py-2 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 font-bold rounded-xl text-xs sm:text-sm border border-amber-500/40 active:scale-95 transition-all"
              >
                {choice.text}
              </button>
            ))}
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-800">
          <div className="text-[11px] text-slate-500 flex items-center gap-2">
            <span>{tl('dialogue.pageOf', { current: activeDialogue.currentPage + 1, total: activeDialogue.pages.length })}</span>
            <span>•</span>
            <span>{tl('dialogue.continue')}</span>
          </div>
