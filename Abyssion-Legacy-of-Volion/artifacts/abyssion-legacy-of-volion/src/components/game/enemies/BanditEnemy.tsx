...
const BANDIT_CONFIG: EnemyConfig = {
  ...
  faction: 'bandit',
  expReward: 30,
  healingDropId: 'small_potion',
  healingDropChance: 0.35,
};


export default function BanditEnemy({ position, name, arenaBounds }: EnemyProps) {
  const config = { ...BANDIT_CONFIG, name: name || BANDIT_CONFIG.name, ...(arenaBounds ? { arenaBounds } : {}) };
  const banditMeshRef = useRef<THREE.Group>(null);
  const swordRef = useRef<THREE.Group>(null);

  const [comboStage, setComboStage] = useState<0 | 1 | 2>(0);
  const comboTimerRef = useRef(0);
  const windupTimerRef = useRef(0);
  const dodgeTimerRef = useRef(0);
  const dodgeCooldownRef = useRef(0);
  const dodgeDirRef = useRef(new THREE.Vector3());
  const _currPos = useRef(new THREE.Vector3());
  const _toPlayer = useRef(new THREE.Vector3());
  const _kbDir = useRef(new THREE.Vector3());
  const _dodgePerp = useRef(new THREE.Vector3());

  const handleCustomUpdate = (ctx: EnemyContext, delta: number): boolean => {
    const { state, setState, rigidBodyRef, playerPos, distToPlayer, setIsDodging, isDodging, enemyId, config } = ctx;

    if (!rigidBodyRef.current || state === 'death') return false;

    const t = rigidBodyRef.current.translation();
    const currPos = _currPos.current.set(t.x, t.y, t.z);
    ...
    if ((state === 'chase' || state === 'attack') && dodgeCooldownRef.current <= 0 && distToPlayer <= 3.8) {
      ...
      if (shouldDodge) {
        ...
        const toPlayer = _toPlayer.current.copy(playerPos).sub(currPos);
        toPlayer.y = 0;
        toPlayer.normalize();
        const side = Math.random() < 0.5 ? 1 : -1;
        dodgeDirRef.current.set(-toPlayer.z * side, 0, toPlayer.x * side).normalize();
    ...
    if (state === 'attack') {
      // Token-ownership validation: if the combo was interrupted (stun/hurt
      // released the token), abort instead of continuing to attack tokenless.
      if (!hasAttackerToken(enemyId)) {
        setComboStage(0);
        windupTimerRef.current = 0;
      ...
      // Face player
      const dir = _toPlayer.current.copy(playerPos).sub(currPos);
      dir.y = 0;
      if (dir.lengthSq() > 0.01 && banditMeshRef.current) {
        banditMeshRef.current.rotation.y = Math.atan2(dir.x, dir.z);
      }

      // Combo Stage 1 — windup then strike
      if (comboStage === 0) {
        if (windupTimerRef.current === 0) {
          // First frame of attack: start windup telegraph (sword raised)
          windupTimerRef.current = config.attackWindup;
          if (swordRef.current) swordRef.current.rotation.y = Math.PI / 2;
          return true;
        }
        windupTimerRef.current -= delta;
        if (windupTimerRef.current > 0) return true;

        // Windup complete — deal damage
        setComboStage(1);
        comboTimerRef.current = 0.3;
        windupTimerRef.current = 0;
    ...
    if (comboStage !== 0 || windupTimerRef.current !== 0) {
      setComboStage(0);
      windupTimerRef.current = 0;
      if (swordRef.current) swordRef.current.rotation.y = 0;
    }
