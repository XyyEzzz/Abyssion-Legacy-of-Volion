...
export default function WolfEnemy({ position, name, arenaBounds }: EnemyProps) {
  const config = { ...WOLF_CONFIG, name: name || WOLF_CONFIG.name, ...(arenaBounds ? { arenaBounds } : {}) };
  ...
  const handleCustomUpdate = (ctx: EnemyContext, delta: number): boolean => {
    const { state, setState, rigidBodyRef, playerPos, distToPlayer, distToSpawn, enemyId, config } = ctx;
    ...
    if (state === 'chase') {
      ...
      if (wolfPhase === 'circling') {
        ...
        if (circleTimerRef.current <= 0) {
          ...
          if (requestAttackerToken(enemyId)) {
            setWolfPhase('lunge');
            setState('attack');
    ...
    if (state !== 'chase' && state !== 'attack') {
      // Leaving chase/attack (stun/hurt/death/leash) — drop the token so the
      // engagement slot is never held stale by a wolf that is no longer attacking.
      if (wolfPhase !== 'none') {
        releaseAttackerToken(enemyId);
        setWolfPhase('none');
      }
