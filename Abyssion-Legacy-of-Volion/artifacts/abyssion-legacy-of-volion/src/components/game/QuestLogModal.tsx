...
export default function QuestLogModal() {
  ...
  // The L-key open/close toggle lives in UI.tsx's single centralized keydown
  // handler (J is the attack key and must not toggle the quest log). This
  // modal only listens for Escape so it can close itself.
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && showQuestLog) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950 backdrop-blur-md pointer-events-auto">
      <div className="w-full max-w-3xl bg-slate-900 border-2 border-slate-700/80 rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[520px] max-h-[90vh] animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="bg-slate-950/80 border-t border-slate-800 px-6 py-2.5 text-[11px] text-slate-500 flex items-center justify-between">
          <span>Press [L] to toggle Quest Journal</span>
