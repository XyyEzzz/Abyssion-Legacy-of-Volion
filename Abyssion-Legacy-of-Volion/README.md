...
The git root is the outer directory, so the full layout, prerequisites and commands are
documented in the repository root's `README.md` (one level up from this file).
...
├── README.md                                      ← the complete documentation
├── Abyssion-Legacy-of-Volion/                     ← INNER (pnpm workspace root, this file's dir)
...
- **pnpm** — the workspace requires it. A `preinstall` guard in this directory's `package.json`
  aborts the install with `Use pnpm instead` if it is run by npm or yarn. There is no `engines`
  field, so any current pnpm release works (the workspace was last run on pnpm 10 and Node 22).
...
fails; use pnpm from this (the workspace) root.

## Install

From this directory (the inner workspace root):
