...
import { EnemyConfig, EnemyProps, releaseAttackerToken } from './types';
...
export default function SlimeEnemy({ position, name }: EnemyProps) {
  const config = { ...SLIME_CONFIG, name: name || SLIME_CONFIG.name };
          ...
          if (impactDist <= 3.2) {
            ...
            if (hitTaken) {
              ...
              if (playerRigidBodyRef.current) {
                ...
                playerRigidBodyRef.current.applyImpulse({ x: kbDir.x * 8, y: 4, z: kbDir.z * 8 }, true);
              }
            }
          }
